...
On 2026-07-23, the 52 `*-name.wav` and `*-phoneme.wav` prompts were re-rendered locally at 0.75x speed with pitch preservation for early learners. Run `npm run generate:slow-letter-audio` to reproduce that processing; it reads the original DittliTTS files from commit `5675c15` and never re-processes the current outputs. The `*-word.wav` prompts remain at their original speed.
